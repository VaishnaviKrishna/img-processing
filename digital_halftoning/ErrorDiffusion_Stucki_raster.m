...
%Implementation: Error diffusion using Stucki matrix (Raster parsing)
%M-file name: ErrorDiffusion_Stucki_raster.m
%Output image: output_error_diffused_Stucki_raster
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1+(N-1)/2:400+(N-1)/2
    for j=1+(N-1)/2:600+(N-1)/2
        ...
        if (added_error_gray_scale_bridge_extended(i,j)>threshold)
            output_error_diffused_Stucki_raster(i-(N-1)/2,j-(N-1)/2)=255;
            else
            output_error_diffused_Stucki_raster(i-(N-1)/2,j-(N-1)/2)=0; 
        end
        error=added_error_gray_scale_bridge_extended(i,j)-output_error_diffused_Stucki_raster(i-(N-1)/2,j-(N-1)/2);
...
figure(3)
imshow(uint8(output_error_diffused_Stucki_raster));
title('Image obtained by Stucki error diffusion using raster parsing');                    